...
export interface PillFormat {
    ...
    pill_ingredient?: (string | string[])[]; // e.g. (thành phần)
    pill_use?: (string | string[])[]; // e.g. (cách dùng)
    pill_dosage?: (string | string[])[]; // e.g. (liều lượng/ liều dùng)
    pill_pharmacology?: (string | string[])[]; // e.g. (cơ chế tác dụng/ dược lý)
    pill_pharmacokinetics?: (string | string[])[]; // e.g. (dược động học/ động học)
